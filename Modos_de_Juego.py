...
class Modos_de_Juego:
    ...
    def __init__(self, state, metodo, profundidad, color, max_jugadas, turno, fichas):
        self.state = state                  
        ...
        self.metodo = metodo
        self.fichas = fichas                # Si vamos a jugar con blancas o con negras
    ...
    def jugarContraMaquina(self, state):
        ...
        if self.fichas == "blancas":                # Jugamos con las fichas blancas
            while self.max_jugadas > 0:             # Mientras queden jugadas disponibles, sigue el juego
            ...
            while self.max_jugadas > 0:         # Mientras queden jugadas disponibles, sigue el juego
                ...
                contador_turnos += 1
                state.inicializaPiezas()            # Inicializamos todas las piezas del tablero
                if self.turno_blancas:              # Si juegan negras
                    if state.isFinal():
                        print("GANAN LAS NEGRAS")
                        sys.exit()
